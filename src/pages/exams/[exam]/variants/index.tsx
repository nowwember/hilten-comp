    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <Link
          ...
          className="btn-ghost inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium hover:bg-[var(--paper-2)] transition-colors"
        >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <h1 className="font-display text-2xl font-bold" style={{ color: 'var(--ink)' }}>{title}</h1>
        <div className="flex gap-2 items-center relative">
          ...
          <button
            ref={monthBtnRef}
            className="btn-ghost ml-2 px-4 py-2 rounded-xl hover:bg-[var(--paper-2)] transition-colors font-medium relative"
            onClick={() => setMonthDropdown(v => !v)}
          >
            Месяц: {selectedMonth}
          </button>
          {monthDropdown && (
            <div className="absolute right-0 top-12 z-20 w-40 rounded-xl shadow-lg py-2" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
              {monthOptions.slice(0, 12).map(month => (
                <button
                  key={month}
                  className={`block w-full text-left px-4 py-2 hover:bg-[var(--paper-2)] transition-colors ${month === selectedMonth ? 'font-bold' : ''}`}
                  style={{ color: month === selectedMonth ? 'var(--red)' : 'var(--ink)' }}
      ...
      {variants.length === 0 ? (
        <div className="text-center py-16" style={{ color: 'var(--ink-soft)' }}>Нет вариантов для выбранных фильтров.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {variants.map(variant => {
            const totalTasks = variant.picks.reduce((sum, p) => sum + p.count, 0);
            return (
              <div key={variant.id} className="group flex flex-col h-full rounded-[var(--radius)] overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-lg" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
                <div className="p-5 flex-1 flex flex-col">
                  <h2 className="font-display font-bold text-lg mb-2 line-clamp-2 min-h-[2.5rem]" style={{ color: 'var(--ink)' }}>{variant.title}</h2>
                  <div className="text-sm mb-2" style={{ color: 'var(--ink-soft)' }}>{variant.tags?.join(', ')}</div>
                  <div className="flex gap-4 font-mono text-xs mb-2" style={{ color: 'var(--ink-soft)' }}>
                  ...
                  <div className="mt-auto flex gap-2 pt-2">
                    <Link
                      ...
                      className="btn-primary flex-1 py-2 px-4 rounded-xl text-center font-medium"
                    >
                      Открыть
                    </Link>
                    <Link
                      href={{ pathname: `/exams/${variant.exam}/variant/${variant.id}/solve`, query: { month: variant.month, mode: variant.mode } }}
                      className="btn-ghost flex-1 py-2 px-4 rounded-xl text-center font-medium hover:bg-[var(--paper-2)] transition"
                    >
                      Начать
                    </Link>
                  </div>
